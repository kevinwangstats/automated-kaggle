...
2. **Baseline Engine**: Evaluates standard frameworks (XGBoost, LightGBM, CatBoost, H2O AutoML) via K-Fold CV to establish a baseline. It generates an initial, ephemeral `train_model.py` ensemble script that is continuously overwritten (unless resuming). It also automatically logs out-of-fold classification metrics and distributions.
3. **Agent Loop**: An orchestrator powered by `litellm`. It uses a **Multi-Message architecture and native File APIs** (e.g., Gemini `upload_file`, OpenAI `file-extract`) to upload the context (`train_model.py`, `EDA.md`) efficiently. The LLM edits the Python training script directly on the dataset branch. Failed iterations retain the broken code so the agent can debug itself; successful iterations are committed and optionally submitted to Kaggle.

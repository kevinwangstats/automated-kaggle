...
def train_and_evaluate(config_path="config.yaml", output_dir="."):
    ...
    try:
        if task == 'classification':
            models.append(('xgb', XGBClassifier(random_state=42, n_jobs=-1)))
        else:
            models.append(('xgb', XGBRegressor(random_state=42, n_jobs=-1)))
    except Exception: pass
    try:
        if task == 'classification':
            models.append(('lgb', LGBMClassifier(random_state=42, verbose=-1, n_jobs=-1)))
        else:
            models.append(('lgb', LGBMRegressor(random_state=42, verbose=-1, n_jobs=-1)))
    except Exception: pass
    try:
        if task == 'classification':
            models.append(('cat', CatBoostClassifier(random_state=42, verbose=0, thread_count=-1)))
        else:
            models.append(('cat', CatBoostRegressor(random_state=42, verbose=0, thread_count=-1)))
    except Exception: pass
    try:
        if task == 'classification':
            models.append(('ridge', RidgeClassifier(random_state=42)))
        else:

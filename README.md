...
- **Kaggle Submissions**: When `auto_kaggle_submit` is enabled, the pipeline will automatically submit to Kaggle *during* the agentic loop whenever an iteration improves the cross-validation score and generates a valid prediction file. The automated submission message sent to Kaggle will dynamically include the short Git commit SHA ID of the codebase that generated it, ensuring precise code provenance.
